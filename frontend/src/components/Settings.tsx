...
const NAV_ITEMS: { key: SettingsTab; label: string; icon: IconName }[] = [
  ...
  { key: "quanti", label: "quanti", icon: "message" },
  ...
  return (
    <>
      ...
      <Section title="files" description="view and edit Quanti's personality and memory — changes take effect on the next Quanti session">
        <div style={{ display: "flex", gap: 0, border: "1px solid var(--border-2)", borderRadius: 9, overflow: "hidden", margin: 14 }}>
          {/* File list sidebar */}
          <div style={{ width: 200, borderRight: "1px solid var(--border-2)", flexShrink: 0 }}>
            {QUANTI_FILES.map((f) => (
              <button
                key={f.name}
                ...
                style={{
                  ...
                  padding: "10px 14px",
                  textAlign: "left",
                  background: activeFile === f.name ? "var(--panel-2)" : "none",
                  border: "none",
              ...
              >
                <div style={{ fontSize: 11, color: activeFile === f.name ? "var(--fg)" : "var(--fg-3)", fontWeight: activeFile === f.name ? 600 : 400 }}>
                  {f.label}
                </div>
                <div style={{ fontSize: 9, color: "var(--fg-4)", marginTop: 2 }}>{f.name}</div>
              </button>
            ))}
          </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 360 }}>
            {!activeFile ? (
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--fg-4)", fontSize: 11, fontFamily: font }}>
              ...
              <>
                <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--panel-2)", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 10, color: "var(--fg-2)", fontFamily: font, flex: 1 }}>{activeFileMeta?.description}</span>
                  <button
                    onClick={saveFile}
                    disabled={saving}
                    style={{
                      padding: "4px 12px",
                      backgroundColor: saved ? "#065F46" : "var(--accent)",
                      border: "none",
                      borderRadius: 4,
                      color: "var(--bg)",
                      fontSize: 10,
