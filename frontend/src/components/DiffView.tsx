...
export function DiffView({ sessionId, sessionName, commitMessagePrefix, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    s.textContent = `
      ...
      .diff-scroll::-webkit-scrollbar-thumb:hover { background: var(--fg-3); }
      .diff-scroll::-webkit-scrollbar-corner { background: var(--bg); }
    `;
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden" style={{ backgroundColor: "var(--bg)", fontFamily: FONT }}>

      {/* ── SIDEBAR ───────────────────────────────────────────────────────── */}
      <div ref={sidebarRef} className="flex flex-col h-full relative"
        style={{ width: sidebarWidth, minWidth: sidebarWidth, borderRight: "1px solid var(--border)" }}>

        <button onClick={onBack} className="flex items-center gap-2 px-4 w-full text-left flex-shrink-0"
          style={{ height: 48, borderBottom: "1px solid var(--border)", backgroundColor: "transparent" }}
          onMouseEnter={e => e.currentTarget.style.backgroundColor = "var(--hover)"}
          onMouseLeave={e => e.currentTarget.style.backgroundColor = "transparent"}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--fg-2)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <line x1="19" y1="12" x2="5" y2="12" /><polyline points="12 19 5 12 12 5" />
          </svg>
          <span style={{ color: "var(--fg)", fontSize: 13, fontWeight: 700 }}>&gt; git diff</span>
        </button>

        <div className="flex items-center gap-2 px-4 flex-shrink-0"
          style={{ height: 32, borderBottom: "1px solid var(--border)", cursor: "pointer" }} onClick={toggleAll}>
          <div style={{ width: 14, height: 14, backgroundColor: "var(--bg)", border: `1px solid ${allChecked || someChecked ? "var(--accent)" : "var(--fg-2)"}`, borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {allChecked  && <span style={{ color: "var(--accent)", fontSize: 9, fontWeight: 700, lineHeight: 1 }}>x</span>}
            {someChecked && <span style={{ color: "var(--accent)", fontSize: 9, fontWeight: 700, lineHeight: 1 }}>−</span>}
          </div>
          <span style={{ color: "var(--fg-2)", fontSize: 11 }}>{loadingFiles ? "loading..." : `${checkedPaths.size} of ${files.length} files`}</span>
        </div>
          ...
          {files.map(file => {
            ...
            return (
              <div key={file.path} className="flex items-center gap-2 px-3"
                style={{ height: 32, cursor: "pointer", backgroundColor: isSelected ? "var(--hover)" : "transparent", borderBottom: "1px solid var(--panel)" }}
                onClick={() => setSelectedFile(file)}
                onMouseEnter={e => { if (!isSelected) e.currentTarget.style.backgroundColor = "var(--panel-2)"; }}
                onMouseLeave={e => { if (!isSelected) e.currentTarget.style.backgroundColor = "transparent"; }}>
                <div style={{ width: 14, height: 14, backgroundColor: "var(--bg)", border: `1px solid ${isChecked ? "var(--accent)" : "var(--fg-2)"}`, borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
                  onClick={e => { e.stopPropagation(); toggleFile(file.path); }}>
                  {isChecked && <span style={{ color: "var(--accent)", fontSize: 9, fontWeight: 700, lineHeight: 1 }}>x</span>}
                </div>
                <span style={{ color: statusColor(file.status), fontSize: 10, fontWeight: 700, flexShrink: 0 }}>{file.status}</span>
                <span style={{ color: isSelected ? "var(--fg)" : "var(--fg-3)", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }} title={file.path}>{file.path}</span>
              </div>
            );
          })}
          {!loadingFiles && files.length === 0 && (
            <div className="flex items-center justify-center h-16"><span style={{ color: "var(--fg-3)", fontSize: 11 }}>no changes</span></div>
          )}
        </div>

        <div style={{ height: 6, cursor: "row-resize", backgroundColor: "var(--bg)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}
          onMouseDown={onFileListResizeStart}
          onMouseEnter={e => e.currentTarget.style.backgroundColor = "var(--hover)"}
          onMouseLeave={e => e.currentTarget.style.backgroundColor = "var(--bg)"}>
          <div style={{ width: 32, height: 2, borderRadius: 1, backgroundColor: "var(--fg-3)" }} />
        </div>

        <div className="flex flex-col gap-3 p-3 overflow-y-auto" style={{ flex: 1, minHeight: 0 }}>
          <label style={{ color: "var(--fg-2)", fontSize: 10 }}>// commit message</label>
          {prefix && <div className="px-2 py-1" style={{ color: "var(--fg-2)", fontSize: 10, backgroundColor: "var(--hover)", border: "1px solid var(--border)" }}>{prefix}</div>}
          <textarea value={commitMessage} onChange={e => setCommitMessage(e.target.value)}
            placeholder="describe your changes..." rows={4}
            style={{ backgroundColor: "var(--hover)", border: "1px solid var(--border)", color: "var(--fg)", fontFamily: FONT, fontSize: 11, padding: "6px 8px", resize: "none", outline: "none", width: "100%", boxSizing: "border-box" }}
            onFocus={e => e.currentTarget.style.borderColor = "var(--accent)"}
            onBlur={e => e.currentTarget.style.borderColor = "var(--border)"} />
          <div className="flex items-center gap-2 cursor-pointer" style={{ color: "var(--fg-2)", fontSize: 11 }} onClick={() => setPushAfter(v => !v)}>
            <div style={{ width: 14, height: 14, backgroundColor: "var(--bg)", border: `1px solid ${pushAfter ? "var(--accent)" : "var(--fg-2)"}`, borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {pushAfter && <span style={{ color: "var(--accent)", fontSize: 9, fontWeight: 700, lineHeight: 1 }}>x</span>}
            </div>
            push after commit
          </div>
          {commitError && <div className="px-2 py-2" style={{ fontSize: 10, backgroundColor: "var(--hover)", border: "1px solid var(--danger)", color: "var(--danger)" }}>{commitError}</div>}
          <button disabled={committing || !(prefix + commitMessage).trim() || checkedPaths.size === 0} onClick={handleCommit}
            style={{ width: "100%", padding: "8px 0", fontSize: 12, fontFamily: FONT, fontWeight: 600,
              backgroundColor: (prefix + commitMessage).trim() && checkedPaths.size > 0 && !committing ? "var(--accent)" : "var(--hover)",
              color: (prefix + commitMessage).trim() && checkedPaths.size > 0 && !committing ? "var(--bg)" : "var(--fg-3)",
      ...
      <div ref={diffAreaRef} className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 flex-shrink-0"
          style={{ height: 40, borderBottom: "1px solid var(--border)", backgroundColor: "var(--panel)" }}>
          ...
          <span style={{ color: "var(--fg)", fontSize: 12, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {selectedFile ? selectedFile.path : "select a file"}
          </span>
          {selectedFile && diffCount > 0 && (
            <span style={{ fontSize: 10, color: "var(--accent)", backgroundColor: "var(--accent-soft)", border: "1px solid var(--accent)", padding: "1px 8px", flexShrink: 0 }}>
        ...
        {selectedFile ? (
          <div className="flex flex-1 min-h-0 overflow-hidden">
            ...
            <div className="flex flex-col overflow-hidden" style={{ flex: beforePanelFlex, minWidth: 0, borderRight: "1px solid var(--border)" }}>
              <div className="flex items-center gap-2 px-4 flex-shrink-0"
                style={{ height: 32, borderBottom: "1px solid var(--border)", backgroundColor: "var(--panel)" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: "var(--danger)" }} />
                <span style={{ color: "var(--fg-2)", fontSize: 11 }}>before</span>
              </div>
              <div ref={beforeScrollRef} className="flex-1 overflow-auto diff-scroll"
                ...
                {loadingDiff ? (
                  <div className="flex items-center justify-center h-full"><span style={{ color: "var(--fg-3)", fontSize: 11 }}>loading...</span></div>
                ) : visualRows.length > 0 ? (
                  <div style={{ minWidth: "max-content" }}>
                    {visualRows.map(row => {
                      ...
                      const bg     = isSpacer ? "var(--panel-2)" : isChanged ? "var(--q-diff-changed-bg)" : isRemoved ? "var(--q-diff-removed-bg)" : "transparent";
                      const numClr = isChanged ? "var(--info)" : isRemoved ? "var(--q-diff-removed-gutter)" : "var(--fg-3)";
                      const mrkClr = isChanged ? "var(--info)" : isRemoved ? "var(--danger)" : "transparent";
                      const mrkSym = isChanged ? "~" : isRemoved ? "−" : " ";
                      const txtClr = isRemoved ? "var(--q-diff-removed-text)" : "var(--fg-3)";
                      return (
                        <div key={row.key} style={{ display: "flex", height: LINE_HEIGHT, backgroundColor: bg }}>
                          <span style={{ width: GUTTER_W, minWidth: GUTTER_W, paddingRight: 8, textAlign: "right", color: numClr, fontSize: 11, fontFamily: FONT, lineHeight: `${LINE_HEIGHT}px`, userSelect: "none", borderRight: "1px solid var(--panel)", flexShrink: 0 }}>
                            {isSpacer ? "" : cell.num}
                          </span>
                  ...
                  <div className="flex items-center justify-center h-full"><span style={{ color: "var(--fg-3)", fontSize: 11 }}>new file</span></div>
                )}
              </div>
            </div>

            {/* Panel resize */}
            <div style={{ width: 6, cursor: "col-resize", backgroundColor: "var(--bg)", borderLeft: "1px solid var(--border)", borderRight: "1px solid var(--border)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}
              onMouseDown={onBeforeResizeStart}
              onMouseEnter={e => e.currentTarget.style.backgroundColor = "var(--hover)"}
              onMouseLeave={e => e.currentTarget.style.backgroundColor = "var(--bg)"}>
              <div style={{ width: 2, height: 32, borderRadius: 1, backgroundColor: "var(--fg-3)" }} />
            </div>
            ...
            <div className="flex flex-col overflow-hidden" style={{ flex: 1 - beforePanelFlex, minWidth: 0, position: "relative" }}>
              <div className="flex items-center gap-2 px-4 flex-shrink-0"
                ...
                <span style={{ color: "var(--fg)", fontSize: 11, fontWeight: 500 }}>current version</span>
                <span style={{ marginLeft: "auto", color: "var(--fg-3)", fontSize: 10 }}>editable</span>
              </div>

              {loadingDiff ? (
                <div className="flex flex-1 items-center justify-center"><span style={{ color: "var(--fg-3)", fontSize: 11 }}>loading...</span></div>
              ) : (
                <div className="flex flex-1 min-h-0 overflow-hidden" style={{ position: "relative", backgroundColor: "var(--bg)" }}>

                  {/* Gutter: line numbers + spacers, synced via scrollTop */}
                  <div ref={afterGutterRef}
                    style={{ width: GUTTER_W + MARKER_W, minWidth: GUTTER_W + MARKER_W, overflow: "hidden", borderRight: "1px solid var(--panel)", flexShrink: 0, backgroundColor: "var(--bg)" }}>
                    {visualRows.map(row => {
                      ...
                      const gutterBg  = isSpacer ? "var(--panel-2)" : "transparent";
                      const numClr    = isChanged ? "var(--info)" : isAdded ? "var(--accent-2)" : "var(--fg-3)";
                      const mrkClr    = isChanged ? "var(--info)" : isAdded ? "var(--accent)" : "transparent";
                  ...
                  <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
                    ...
                    <div ref={afterBgRef}
                      ...
                      {visualRows.map(row => {
                        ...
                        const isChanged = !isSpacer && cell.type === "added" && row.before !== null;
                        const isAdded   = !isSpacer && cell.type === "added" && row.before === null;
                        const bg = isSpacer ? "var(--panel-2)" : isChanged ? "var(--q-diff-changed-bg)" : isAdded ? "var(--accent-soft)" : "transparent";
                        return (
              ...
              {visualRows.length > 0 && (
                <div style={{ position: "absolute", top: 32, right: 0, bottom: 0, width: 7, backgroundColor: "var(--panel)", zIndex: 30, pointerEvents: "none" }}>
                  {visualRows.map((row, i) => {
                    ...
                    const color = isChanged ? "var(--info)" : isAdded ? "var(--accent)" : "var(--danger)";
