...
13. "No speech was heard" timeout + orb dead after turn 1: classic WebKit symptom of the AudioContext idling into suspended/interrupted between turns (starves the VAD). Fix (this run): (a) silent looping keep-alive AudioBufferSourceNode (startKeepAlive in doInit+resumeContext, stopKeepAlive in teardownCapture) so the context never idles out; (b) listen timeout is now reported as an EMPTY transcript (voiceBridge) so a missed utterance gives the agent the "keep going" nudge instead of a hard error that ends the loop. Frontend-only. Verified: tsc ✓, vite build ✓, 11/11 Playwright ✓.
