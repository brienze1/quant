...
// Grant WebKitGTK media-capture (getUserMedia) permission requests so the
// embedded webview can access the microphone/camera. The OS still controls the
// physical device; this only allows the in-page permission to be granted instead
// of WebKitGTK's default deny. Mirrors the macOS WKUIDelegate media-capture grant.
...
GtkWidget *SetupWebview(void *contentManager, GtkWindow *window, int hideWindowOnClose, int gpuPolicy, int disableWebViewDragAndDrop, int enableDragAndDrop)
{
    ...
    // Enable getUserMedia (microphone/camera) support in the embedded webview and
    // auto-grant the in-page permission request (see onPermissionRequest above).
    webkit_settings_set_enable_media_stream(settings, TRUE);
    g_signal_connect(G_OBJECT(webview), "permission-request", G_CALLBACK(onPermissionRequest), NULL);
