...
- **WI-32 · Per-note color picker.** `types.MindmapNode` += `color?: string`; `EditDraft` += `color`. In `EditNodeModal` add a **preset swatch palette** (a row of ~7 color buttons + a "default/none" option) — **NOT** native `<input type=color>` (unreliable in WKWebView, same class of limitation as `window.prompt`). Palette = theme-friendly fixed hex (e.g. amber/blue/green/red/purple/pink/teal). `submitEdit` includes `color: d.color`. Render: `NoteNode` applies the color to the sticky background/border via inline `style` (tint via `color-mix` or rgba), falling back to `var(--q-warning)` when empty; `StatusNode`'s `.node-note` applies the color to its left-border/tint, falling back to `var(--q-blue)`. Show the current swatch when editing an existing node. The `mindmap:updated` snapshot already carries `color` (WI-30) so agent/user colors reconcile by id like other fields. *Verify:* pick a color for a note → persists, shows on the canvas, survives reload + a concurrent agent snapshot; empty color = themed default.
