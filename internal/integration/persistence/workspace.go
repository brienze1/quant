...
const workspaceColumns = `id, name, claude_config_path, mcp_config_path, crew_cli_command, created_at, updated_at`

func scanWorkspaceRow(scanner interface{ Scan(...any) error }) (pdto.WorkspaceRow, error) {
	var row pdto.WorkspaceRow
	err := scanner.Scan(
		&row.ID, &row.Name, &row.ClaudeConfigPath, &row.McpConfigPath, &row.CrewCliCommand, &row.CreatedAt, &row.UpdatedAt,
	)
	return row, err
}
...
func (p *workspacePersistence) SaveWorkspace(workspace entity.Workspace) error {
	...
	query := `INSERT INTO workspaces (id, name, claude_config_path, mcp_config_path, crew_cli_command, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)`
...
func (p *workspacePersistence) UpdateWorkspace(workspace entity.Workspace) error {
	...
	query := `UPDATE workspaces SET name = ?, claude_config_path = ?, mcp_config_path = ?, crew_cli_command = ?, updated_at = ? WHERE id = ?`

	result, err := p.db.Exec(query, row.Name, row.ClaudeConfigPath, row.McpConfigPath, row.CrewCliCommand, row.UpdatedAt, row.ID)
	if err != nil {
